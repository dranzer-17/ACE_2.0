...
import { Calendar, List, Trophy, Star, Users, Plus, Edit } from 'lucide-react';
...
export default function FacultyEventsPage() {
  ...
  // Faculty can see all events and manage their own
  const facultyManagedEvents = eventsData.filter(event => 
    event.organizer.includes('Department') || 
    event.organizer.includes('Committee') ||
    event.organizer.includes('Faculty')
  );
  ...
  return (
    <div className="container mx-auto px-4 py-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">Faculty Events Dashboard</h1>
          <p className="text-gray-600 mt-2">
            Manage and participate in college events, workshops, and academic activities
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <Badge variant="outline" className="text-blue-600 border-blue-200">
            {facultyManagedEvents.length} Managed
          </Badge>
        </div>
      </div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold">{facultyManagedEvents.length}</p>
                <p className="text-sm text-gray-600">Managed Events</p>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            {/* Filters */}
            <EventFilters
              events={activeTab === 'managed' ? facultyManagedEvents : (activeTab === 'all' ? eventsData : currentEvents)}
              filteredEvents={activeTab === 'managed' ? facultyManagedEvents : currentEvents}
              onFilterChange={setFilters}
            />

            {/* Events Grid */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {(activeTab === 'managed' ? facultyManagedEvents : currentEvents).map((event) => (
                <div key={event.id} className="relative">
                  ...
                  />
                  {/* Faculty Edit Button */}
                  {facultyManagedEvents.includes(event) && (
                    <Button
                      size="sm"
                      variant="outline"
